...
build_platform() {
    ...
    if [ $? -eq 0 ]; then
        # Copy Rust library
        if [ "$GOOS" = "linux" ]; then
            cp target/release/libfilemanager.so ${PLATFORM_DIR}/
        elif [ "$GOOS" = "darwin" ]; then
            cp target/release/libfilemanager.dylib ${PLATFORM_DIR}/ 2>/dev/null || cp target/release/libfilemanager.so ${PLATFORM_DIR}/libfilemanager.dylib
        elif [ "$GOOS" = "windows" ]; then
            cp target/release/filemanager.dll ${PLATFORM_DIR}/ 2>/dev/null || echo "Note: Windows build requires manual DLL"
        fi
